...
\section{CycleGAN与DANN的对比选型}
  \subsection{跨域迁移面临的挑战}
    ...
    而目标域为本地收集的500例孤立ACL撕裂患者影像，采用的是带有脂肪抑制的快速自旋回波（Fat-suppressed FSE）序列。
    这两种序列在灰度分布、组织对比度和噪声模式上存在巨大的域偏移。
    如果直接将OAI上训练的U-Net应用于孤立前交叉韧带和多发韧带损伤的FSE数据，由于数据分布不匹配，分割精度会发生断崖式下降。
  ...
  \subsection{基于特征级对齐的选择：DANN}
    ...
    \subsubsection{针对膝关节MRI的架构改进}
      ...
      \begin{itemize}
        \item 实例归一化（Instance Normalization, IN）：将ResNet34 主干网络中的批归一化（Batch Normalization）全部替换为实例归一化。因为实例归一化是在单个样本内部进行特征归一化，能极其有效地过滤掉不同MRI扫描仪及序列带来的局部对比度风格差异，迫使模型将注意力集中在骨与软骨的纯粹几何与拓扑形状上。
...
\section{改进 DANN 网络架构设计}
  ...
  \subsection{多尺度上下文感知}
    ...
    研究表明，这种样本级的归一化能够极其有效地过滤掉由成像物理过程带来的对比度风格差异，从而迫使网络在特征提取阶段忽略灰度变化，
    将注意力集中于骨骼与软骨结构的纯粹几何形态与边缘特征上，为后续的域对齐奠定了坚实基础。
  ...
  \subsection{基于损失比率的动态冻结对抗策略}
    ...
    \subsubsection{动态冻结指标的定义与执行逻辑}
      ...
      \begin{figure}[!htp]
        ...
        \label{fig:dann_loss}
      \end{figure}
    \subsubsection{纳什均衡状态的实现与分析}
      通过引入该动态冻结策略，本研究成功克服了常规 UDA 训练中常见的崩溃现象。
      在实际的训练监控中可以观察到，在二分类（源域 vs 目标域）的对抗博弈中，随着训练的深入，
      域判别损失（$L_{adv}$）不再趋近于 $0$，而是被牢牢锁定在 $0.693$（即 $-\ln(0.5)$）附近震荡。
      \par 在对抗学习的数学理论中，$0.693$ 正是二值交叉熵损失（Binary Cross-Entropy Loss）的全局最优解。
      这在数学上严密地证明了网络达到了最佳的纳什均衡状态：域判别器此时的输出概率始终在 $0.5$ 徘徊，它只能靠“随机猜测”来判断特征的来源。
      这印证了改进型的 ResNet34 特征提取器已经成功剥离了 DESS 与 FSE 序列在磁共振物理层面的对比度信息，
      将源域与目标域在深层空间中融合为了统一的、只包含骨与软骨拓扑几何特征的域不变流形（Domain-invariant Manifold）。
\section{本章实验与结果分析}
  为了验证上述改进型 DANN 网络架构及其动态对抗训练策略在临床无标注数据上的实际效能，
  本节在前期构建的黄金标准测试集上进行了严格的定量与定性分析，并对训练过程中的损失动力学进行了复盘。
  \subsection{UDA任务中判别器收敛过快导致生成器梯度消失}
