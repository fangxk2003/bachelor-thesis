...
\section{实验数据集介绍}
  ...
  \subsection{孤立前交叉韧带损伤数据集与多发韧带损伤数据集}
    \subsubsection{临床目标域数据分布与无监督痛点}
      ...
      这种典型的“有数据、无标签”特征，构成了本研究引入无监督域适应（UDA）框架的重要动因。
      \begin{table}[!hpt]
        \caption{临床目标域数据分布统计}
    ...
    \subsubsection{代表性测试集的精细化人工标注}
      尽管本研究构建的改进型DANN深度学习管线在网络训练与特征学习阶段不使用目标域标签，
      但为了在模型测试阶段提供相对客观的性能评估基准（Ground Truth），仍需构建一个小规模本地测试集。
      \par 为此，本研究在665例目标域数据中，综合考虑了损伤类型、病变严重程度以及影像解剖特征的多样性，筛选出具有代表性的30例患者影像。
