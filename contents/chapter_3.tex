...
\section{实验数据集介绍}
  \subsection{OAI-ZIB 数据集}
    骨关节炎倡议-柏林祖斯研究所（Osteoarthritis Initiative-Zuse Institute Berlin, OAI-ZIB）数据集是膝关节医学影像研究领域中的“金标准”数据集之一\cite{ambellanAutomatedSegmentationKnee2019}。
    ...
    \subsubsection{OAI-ZIB 数据集的分割掩膜与关键参数}
      ...
      \par 该数据集的重要价值在于其提供了开源领域内质量较高的骨与软骨三维分割掩膜（Ground Truth）。
  ...
  \subsection{孤立前交叉韧带损伤数据集与多发韧带损伤数据集}
    \subsubsection{临床目标域数据分布与无监督痛点}
      ...
      这种典型的“有数据、无标签”特征，构成了本研究引入无监督域适应（UDA）框架与自监督超分辨率重建算法的重要动因。
      \begin{table}[!hpt]
        \caption{临床目标域数据分布统计}
    ...
    \subsubsection{代表性测试集的精细化人工标注}
      尽管本研究构建的深度学习管线（如改进型DANN与基于深度图像先验的多视图融合）在网络训练与特征学习阶段不使用目标域标签，
      但为了在模型测试阶段提供相对客观的性能评估基准（Ground Truth），仍需构建一个小规模本地测试集。
      \par 为此，本研究在665例目标域数据中，综合考虑了损伤机制、病变严重程度以及影像解剖特征的多样性，筛选出具有代表性的30例患者影像。
    ...
    \subsubsection{标注数据在研究管线中的定位}
      ...
      其主要价值在于模型推理阶段：第四章基于其中150张质控合格的矢状位切片，计算网络预测掩膜与人工标注掩膜之间的Dice相似系数、平均表面距离（ASD）等空间重叠度量指标，
...
\section{图像预处理流程}
  \subsection{格式的转换与数据清洗}
    ...
    DICOM格式不仅包含了图像像素矩阵，还封装了较为复杂的患者隐私信息与扫描仪物理参数。
    ...
    \subsubsection{临床DICOM数据的清洗与NIfTI体积化重构}
      ...
      这种轻量级的三维格式提升了后续数据加载和张量（Tensor）运算的效率，并有效剔除了与分割任务无关的冗余元数据。
    \subsubsection{基于Spacing的空间重采样与尺寸统一}
      ...
      这一尺寸的统一不仅满足了网络输入要求，也与源域（OAI-ZIB）的尺寸保持一致，为后续域对抗神经网络（DANN）的跨域特征匹配提供了空间基准。
    \subsubsection{偏置场校正与三维全局灰度归一化}
      ...
      这一归一化策略有助于减弱不同临床扫描仪之间的“对比度风格”差异，为模型训练和后续BML候选高信号区域提取提供更一致的灰度输入。
  \subsection{N4偏置场校正}
    ...
    \subsubsection{N4ITK算法的核心数学原理}
      ...
      \begin{itemize}
        ...
        \item 随后，N4 算法改进了原 N3 算法中容易产生伪影的常规平滑策略，引入了稳健的 B样条（B-spline）近似算法。通过在三维空间中构建多分辨率的 B样条控制点网格（Control point grid），算法对初步估计的偏置场进行低频空间平滑拟合。经过多次“直方图锐化-B样条平滑”的迭代，网络最终收敛并剥离出较为平滑的全局偏置场 $\log f(x)$，从而还原出真实的组织灰度分布 $u(x)$。
      \end{itemize}
    \subsubsection{本研究中的参数配置与实现目标}
      ...
      \begin{itemize}
        \item 降低类内方差，以加速网络收敛：将同一患者、同一序列内不同空间位置的同类组织灰度拉回到同一基准线上，降低了 DANN 网络在特征提取阶段的拟合难度，使得网络能够更多关注软骨的拓扑流形而非局部亮度变化。
        \item 为后续的 BML 候选高信号区域提取奠定基础：在本研究中，BML相关指标采用基于统计学阈值的方法从预处理后的FSE影像中初步圈定高信号候选区域。N4 校正有效降低了图像边缘暗区的干扰，使得全局阈值或自适应阈值能够更稳定地识别表现为高信号的疑似水肿区域。 
        \item 统一跨模态域适应的灰度语义：源域 OAI-ZIB 数据集（DESS序列）由于扫描规范，其磁场均匀性较好。对目标临床 FSE 数据进行 N4 校正，有助于缩小源域与目标域在底层图像统计学特性上的差异，提升 DANN 中对抗判别器的对齐稳定性。
      \end{itemize}
      \begin{figure}[!htp]
        \centering
  ...
  \subsection{基于分辨率的空间重采样与 3D 全局归一化}
    ...
    \subsubsection{物理空间域的重采样（Resampling）与尺寸对齐}
      ...
      \subsubsection{图像特征域的 3D 全局归一化}
        ...
        这一处理有助于降低不同机器间的“对比度风格”差异，稳定模型前向传播中的激活值分布，
        ...
        这为第四章改进型 DANN 网络进行跨域特征学习与分割提供了数据基础。
