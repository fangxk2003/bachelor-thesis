...
\section{膝关节组织深度学习分割方法}
  ...
  \subsection{面向三维体积的V-Net与损失函数优化}
    ...
    \par 此外，V-Net 在医学图像分割领域的一个重要贡献是首次提出了基于 Dice 相似系数的损失函数（Dice Loss）。
  ...
  \subsection{网络变体、自适应框架与多平面策略}
    ...
    近年来，Isensee 等人提出的 nnU-Net（No-New-Net）框架更是通过自适应配置预处理、网络拓扑及后处理策略，在包括膝关节在内的数十项医学图像分割挑战中确立了最新的性能基准（State-of-the-Art, SOTA）\cite{isenseeNnUNetSelfconfiguringMethod2021}。
...
\section{多视图MRI融合与超分辨率重建}
  ...
  \subsection{基于刚性配准与深度图像先验（DIP）的无监督多视图融合重建}
    ...
    \subsubsection{深度图像先验（DIP）的无监督范式转换}
      ...
      为本文后续探索基于配准联合DIP的膝关节候选融合管线提供了理论参考。
