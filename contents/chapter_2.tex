...
\section{膝关节组织深度学习分割方法}
  近年来，深度学习（Deep Learning, DL）逐渐应用于医学图像处理，
  其中卷积神经网络（Convolutional Neural Network, CNN）在膝关节磁共振图像（MRI）分割中已有较多研究。
  膝关节关键组织（如关节软骨、半月板、韧带和骨骼）的准确分割，是后续骨关节炎定量分析与辅助诊断的基础。
  目前，基于U-Net和V-Net架构及其衍生变体的深度学习方法构成了该领域的研究主流。
  \subsection{U-Net架构及其在膝关节二维分割中的应用}
    U-Net 由 Ronneberger 等人于2015年首次提出，其主要特征是对称的“编码器-解码器（Encoder-Decoder）”U型结构以及跳跃连接（Skip Connections）机制\cite{ronnebergerUNetConvolutionalNetworks2015c}。
    编码器通过连续的卷积与下采样操作提取图像的高维抽象语义特征，而解码器则通过上采样逐步恢复图像的空间分辨率。
    跳跃连接将编码器中保留空间位置信息的浅层特征图与解码器中的深层语义特征图在通道维度上拼接，可保留医学图像中的细小边界信息。
    \begin{figure}[!htp]
      \centering
  ...
  \subsection{面向三维体积的V-Net与损失函数优化}
    ...
    \par 此外，V-Net 在医学图像分割中的一个特点是较早系统采用了基于 Dice 相似系数的损失函数（Dice Loss）。
  ...
  \subsection{网络变体、自适应框架与多平面策略}
    ...
    近年来，Isensee 等人提出的 nnU-Net（No-New-Net）框架通过自适应配置预处理、网络拓扑及后处理策略，在包括膝关节在内的多项医学图像分割任务中成为常用性能基准\cite{isenseeNnUNetSelfconfiguringMethod2021}。
    \par 然而，在处理高分辨率膝关节三维MRI时，直接采用全3D网络（如3D U-Net）面临着较大的图形处理器（Graphics Processing Unit, GPU）显存开销限制，而传统的单平面2D网络又缺乏空间上下文信息。
    因此，“2.5D切片融合”与“多平面网络（Multi-Planar Networks）”成为平衡计算成本与分割精度的常见方向。
...
\section{医学图像的域适应技术}
  \subsection{基于图像翻译的生成式模型 CycleGAN}
    图像翻译（Image Translation）是图像域适应领域（Domain Adaptation, DA） 的常见分支。其中大部分技术都需要成对的训练数据，但这在医学图像领域较难获得。
  ...
  \subsection{基于特征对齐的模型 DANN}
    \subsubsection{研究背景与理论基础}
      ...
      \par 域对抗神经网络（Domain-Adversarial Neural Networks, DANN）\cite{ganinDomainAdversarialTrainingNeural2017}的理论基础包括 Ben-David 等人提出的H-散度理论\cite{ben-davidTheoryLearningDifferent2010}。
      该理论指出：如果一个学习算法无法识别输入数据是来自源域还是目标域，那么这种特征表示对于跨域迁移就是有效的。
    \subsubsection{网络结构}
      DANN 将领域自适应与深度特征学习放在同一个训练过程中，其网络架构主要由三个部分组成：
      \begin{itemize}
      ...
      \end{itemize}
    \subsubsection{梯度反转层}
      DANN 的主要技术特点在于引入梯度反转层（Gradient Reversal Layer, GRL），并将其放置在特征提取器 $G_f$ 和域分类器 $G_d$ 之间。
      \begin{itemize}
      ...
      \end{itemize}
      \par 这一设计使基于反向传播的随机梯度下降算法（Stochastic Gradient Descent, SGD）
      可以直接用于求解对抗性的 Min-Max 优化问题。
...
\section{多视图MRI融合与超分辨率重建}
  在膝关节的临床影像学检查中，由于成像时间与信噪比（Signal-to-Noise Ratio, SNR）的限制，常规的二维快速自旋回波（2D FSE）序列通常具有较高的面内分辨率，
  ...
  \subsection{图像超分辨率与切片到体积重构（Slice-to-Volume Reconstruction, SVR）原理}
    超分辨率重建技术的基本思路是从低分辨率（Low-resolution）或欠采样数据中恢复高分辨率（High-resolution）图像细节。
    其中，SVR是医学图像超分辨率领域的一项关键技术。
    ...
    这类基于物理模型的重构方法可将离散二维切片信息映射到三维体素网格。
  ...
  \subsection{基于刚性配准与深度图像先验（Deep Image Prior, DIP）的无监督多视图融合重建}
    ...
    \subsubsection{传统SVR方法与多视图优化的局限性}
      ...
      以领域内广泛使用的开源工具NiftyMIC为例，其基本工作流依赖于两步迭代法：
      ...
      这种离散化会使计算复杂度和内存消耗随体积大小快速增长，并且在面对形态较复杂的膝关节软骨时，容易在正交平面的交叉处产生网格状伪影。
    \subsubsection{深度学习的引入与监督数据的瓶颈}
      将卷积神经网络（CNN）引入超分辨率重建，可以减轻传统优化方法的计算负担。但是绝大多数现有深度学习超分辨率模型属于监督学习，
      训练通常依赖于大量且精确对齐的高分辨率与低分辨率图像对（HR-LR pairs）。
      在真实的膝关节临床扫描中，由于患者的微小运动，获取精确对齐的多平面各向同性高保真金标准存在较大困难。
      在缺少此类监督数据时，常规深度学习模型在实际应用中容易出现过拟合或跨域伪影。
    \subsubsection{深度图像先验（DIP）的无监督重建思路}
      为了降低监督学习对成对数据的依赖，Ulyanov等人提出了一种无监督框架——深度图像先验（DIP）。
      ...
      因此，该方法可能利用矢状、冠状和轴状三个正交视角中的互补信息。
      \par DIP技术为无监督多视图超分辨率重建提供了一条无需外部标签的候选路径。
      其减少跨平面层间断层伪影、恢复平滑解剖边界的能力，仍需要结合具体数据进行验证。

\section{现有研究的不足与本文切入点}
  结合上述研究，膝关节医学影像的深度学习分割与重建在真实临床环境中仍面临以下问题：
  \begin{itemize}
    \item 标注依赖较强：主流的U-Net、V-Net等模型通常需要大规模体素级标注数据。此类标注获取成本较高，也难以覆盖临床场景中复杂多样的病理变异。
    ...
    \item 传统三维重建方法计算开销较大：在消除FSE序列部分容积效应的三维重建任务中，传统的切片到体积重构（SVR，如NiftyMIC）依赖离散体素网格与物理退化模型，计算开销随体积规模增长较快，且对病变软骨等极薄曲面的边缘保护能力不足。
  \end{itemize}
