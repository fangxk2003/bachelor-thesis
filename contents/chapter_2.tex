...
\section{膝关节组织深度学习分割方法}
  近年来，深度学习（Deep Learning, DL）技术在医学图像处理领域取得了突破性进展，
  尤其是卷积神经网络（Convolutional Neural Network, CNN）在膝关节磁共振图像（MRI）分割中展现出卓越的性能。
  ...
  \subsection{U-Net架构及其在膝关节二维分割中的应用}
    U-Net 由 Ronneberger 等人于2015年首次提出，其最显著的特征是对称的“编码器-解码器（Encoder-Decoder）”U型结构以及跳跃连接（Skip Connections）机制\cite{ronnebergerUNetConvolutionalNetworks2015b}。
    ...
    \begin{figure}[!htp]
      ...
      \caption{U-Net架构示意图。图片来源：Ronneberger 等人\cite{ronnebergerUNetConvolutionalNetworks2015b}。}
      \label{fig:unet}
    \end{figure}
    \par 在膝关节MRI的分割应用中，由于软骨呈现极薄的曲面分布，U-Net展现出了极高的边界还原能力。
    例如，Norman 等人成功利用 2D U-Net 卷积神经网络对膝关节MRI数据进行了自动化的软骨和半月板分割，为后续的形态学与弛豫时间（Relaxometry）定量分析提供了可靠的掩膜基础\cite{bUse2DUNet2018}。
  \subsection{面向三维体积的V-Net与损失函数优化}
    由于临床膝关节MRI数据（如DESS或FSE序列）通常是具有层间物理间距的三维体积数据，直接对其进行逐层2D分割会割裂Z轴方向上的解剖连续性。
    为解决这一问题，Milletari 等人提出了完全基于三维卷积的 V-Net 架构\cite{milletariVNetFullyConvolutional2016}。
    V-Net 继承了 U-Net 的整体拓扑，但将所有的2D操作升级为3D操作，并在卷积块中引入了残差学习（Residual Learning）以加速网络收敛。
    \par 此外，V-Net 在医学图像分割领域的一个重大贡献是首次提出了基于 Dice 相似系数的损失函数（Dice Loss）。
    在膝关节影像中，软骨与骨髓病变（BML）的体素数量往往远少于背景及其他组织，存在极端的类别不平衡（Class Imbalance）问题。
    传统的交叉熵损失会主导网络偏向于预测背景，而 Dice 损失通过全局计算预测掩膜与真实标签的重叠率，极大地提升了网络对软骨等小目标结构的分割灵敏度。
  \subsection{网络变体、自适应框架与多平面策略}
    随着算法的演进，研究者在基础 U-Net 的框架上衍生出了众多变体。
    例如，研究表明基于 SegNet 变体的编解码器架构在骨与软骨的分割中同样能够实现高精度的特征还原。
    近年来，Isensee 等人提出的 nnU-Net（No-New-Net）框架更是通过自适应配置预处理、网络拓扑及后处理策略，在包括膝关节在内的数十项医学图像分割挑战中确立了最新的性能基准（State-of-the-Art, SOTA）\cite{isenseeNnUNetSelfadaptingFramework2018}。
    \par 然而，在处理高分辨率膝关节三维MRI时，直接采用全3D网络（如3D U-Net）面临着巨大的GPU显存开销限制，而传统的单平面2D网络又缺乏空间上下文信息。
    因此，“2.5D切片融合”与“多平面网络（Multi-Planar Networks）”成为平衡计算成本与分割精度的重要方向。
    例如，Multi-Planar U-Net 等架构通过将包含目标体素的二维切片沿矢状面、冠状面和轴状面（Sagittal, Coronal, Axial）三个正交视角分别提取并输入平行的 2D 网络，最后在输出端进行多视角预测特征的三维融合（Multi-view Fusion）\cite{ryuMultiplanar25DUNet} 。
...
\section{医学图像的域适应技术}
  \subsection{基于图像翻译的生成式模型 CycleGAN}
    ...
    CycleGAN（Cycle-Consistent Adversarial Networks）\cite{zhuUnpairedImagetoImageTranslation2020}凭借无需成对的训练数据的特性，被广泛应用在跨模态或跨中心的医学影像迁移（如 MRI 到 CT\cite{wolterinkDeepMRCT2017}）。
    ...
    \begin{figure}[!htp]
      ...
      \caption{CycleGAN架构示意图。图片来源：Zhu 等人\cite{zhuUnpairedImagetoImageTranslation2020}。}
      \label{fig:cyclegan}
    \end{figure}
    \subsubsection{核心挑战}
      无配对数据域迁移的主要挑战在于，单向的映射 $G:X \to Y$ 是高度欠约束的，存在无数种映射方式可以使输出分布与目标分布相同，这极易导致“模式崩溃（Mode Collapse）”的问题，即所有输入都被映射到同一张输出图像上。
    \subsubsection{核心架构设计}
      为了解决上述欠约束问题，CycleGAN 创新性地引入了两个映射函数和两个判别器，并结合了“循环一致性”的结构假设。
      双生成器（映射函数）：
      \begin{itemize}
    ...
    \subsubsection{在骨关节医学影像领域的应用}
      ...
      同时，ACL 损伤相关的病理特征（如 BML 信号）在风格迁移过程中可能被视为噪声后抹除，或者引入不真实的伪影，严重影响后续针对 BML 临床指标的下游分割和诊断任务。
  \subsection{基于特征对齐的模型 DANN}
    \subsubsection{研究背景与核心理论}
      ...
      \par 域对抗神经网络（Domain-Adversarial Neural Networks, DANN）\cite{ganinDomainAdversarialTrainingNeural2016}的核心理论基础是 Ben-David 等人提出的H-散度理论\cite{ben-davidTheoryLearningDifferent2010}。
...
\section{多视图MRI融合与超分辨率重建}
  ...
  这种各向异性（Anisotropic）的体素分布会导致严重的部分容积效应（Partial Volume Effects），
  使得图像无法在任意平面上进行高质量的多平面重组，从而极大地限制了对极薄的关节软骨进行精确的三维形态学评估与体积、厚度测量。
  为了突破这一物理扫描瓶颈，多视图融合与超分辨率重建（Super-Resolution Reconstruction, SRR）技术应运而生\cite{lomurnoImprovingMultiViewStereo2022}。
  ...
  \subsection{基于刚性配准与深度图像先验（DIP）的无监督多视图融合重建}
    ...
    \subsubsection{传统SVR方法与多视图优化的局限性}
      ...
      这种离散化不仅导致计算复杂度和内存消耗随体积大小呈指数级增长，而且在面对形态极其复杂的膝关节软骨时，容易在正交平面的交叉处产生网格状伪影。
    \subsubsection{深度学习的引入与监督数据的瓶颈}
      将卷积神经网络（CNN）引入到超分辨率重建中，能够减轻传统优化的计算负担情况。但是绝大多数现有的深度学习超分辨率模型都是监督学习，
      训练极度依赖于大量且精确对齐的高分辨率与低分辨率图像对（HR-LR pairs）。
      在真实的膝关节临床扫描中，由于患者的微小运动，获取完全对齐的多平面各向同性的高保真金标准是几乎不可能的。
      这种监督数据的缺失会因此导致常规深度学习模型在实际应用中，面临过拟合风险和跨域测试时的光谱伪影。
    \subsubsection{深度图像先验（DIP）的无监督范式转换}
      为了打破监督学习的数据依赖瓶颈，Ulyanov等人提出了一种革命性的无监督框架——深度图像先验（Deep Image Prior, DIP）。
      该理论证明，卷积神经网络自身的拓扑架构（如U-Net）在随机初始化状态下，就已经内化了极其强大的底层图像统计先验信息。
      DIP不需要任何外部成对的训练数据，仅仅利用输入的退化图像就能进行从头优化（De novo optimization）\cite{ulyanovDeepImagePrior2020}。
      \par 在MRI超分辨率重建任务中，DIP会把一个完全随机初始化的网络作为先验的约束器，然后通过定义一个模拟切片降采样的前向物理退化模型，
      来迭代最小化网络生成的三维体积与输入的各向异性的二维切片之间的重建损失。
      在此过程中网络可以被证明等效为一种平滑与锐化的正则化器（Regularizer）。
      因为DIP在优化过程中天然地抗拒高频噪声而倾向于拟合自然解剖的低频连续结构，
      所以它能够非常高效地融合来自矢状、冠状和轴状三个正交视角的互补信息。
      \par 综上所述，DIP技术为医学图像的无监督多视图超分辨率重建提供了一条无需外部标签的全新路径。
      它在消除跨平面层间断层伪影、恢复平滑解剖边界方面有非常卓越的潜力，
      为本文后续构建基于配准联合DIP的高分辨率膝关节融合管线提供了坚实的理论基础。

\section{现有研究的不足与本文切入点}
  综合上述文献分析，目前针对膝关节医学影像的深度学习分割与重建技术虽已取得显著进展，但在应对真实临床环境时仍存在三大核心瓶颈：
  \begin{itemize}
    \item 过度依赖强监督范式：主流的U-Net、V-Net等模型均需要海量且精确配对的体素级标注数据。这种强监督范式不仅获取成本极其高昂，且难以覆盖临床真实场景中层出不穷的病理变异。
    \item 生成式域适应破坏解剖保真度：在解决数据域分布偏移时，当前部分研究倾向于使用CycleGAN等基于图像翻译的无监督方法。然而，此类生成式模型缺乏对局部关键解剖结构拓扑的严格约束，在跨序列翻译过程中极易产生“幻构”伪影，甚至抹除BML等隐匿的病理高信号，导致后续临床定量分析失去医学严谨性。
    \item 传统三维重建方法计算僵化：在消除FSE序列部分容积效应的三维重建任务中，传统的切片到体积重构（SVR，如NiftyMIC）高度依赖离散的体素网格与复杂的物理退化模型，不仅计算开销呈指数级增长，且对病变软骨等极薄曲面的边缘保护能力不足。
  \end{itemize}