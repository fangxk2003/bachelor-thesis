...
\section{膝关节组织深度学习分割方法}
  ...
  \subsection{网络变体、自适应框架与多平面策略}
    ...
    近年来，Isensee 等人提出的 nnU-Net（No-New-Net）框架更是通过自适应配置预处理、网络拓扑及后处理策略，在包括膝关节在内的数十项医学图像分割挑战中确立了最新的性能基准（State-of-the-Art, SOTA）\cite{isenseeAbstractNnUNetSelfadapting2019}。
...
\section{医学图像的域适应技术}
  \subsection{基于图像翻译的生成式模型 CycleGAN}
    ...
    CycleGAN（Cycle-Consistent Adversarial Networks）\cite{zhuUnpairedImagetoImageTranslation2017}凭借无需成对的训练数据的特性，被广泛应用在跨模态或跨中心的医学影像迁移（如 MRI 到 CT\cite{wolterinkDeepMRCT2017a}）。
